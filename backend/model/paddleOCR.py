...
# #     model = LatexOCR()
# #     image = plt.imread(image_path)  # 请替换'your_image.png'为你的图片路径
...
# #         position[2][1] += 0.01
# #         position[3][1] += 0.01
# #         item.append(position.tolist())
# #         item.append((model(img), data['score']))
# #         result1.append(item)
# #     # 隐藏坐标轴
# #     ax.axis('off')
# #     # 显示原始图像
...
# #     result = ocr.ocr('output_image.png', cls=True)
...
def image_to_text(image_path):
    ...
    print(image_path+str(len(result)))
